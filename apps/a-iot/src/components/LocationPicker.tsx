...
export default function LocationPicker({
  ...
  useEffect(() => {
    ...
    const initializeViewer = async () => {
      try {
        const viewer = new CesiumViewer(cesiumContainerRef.current!, DEFAULT_VIEWER_OPTIONS)
        ...
        handler.setInputAction((movement: ScreenSpaceEventHandler.MotionEvent) => {
          if (isDragging && dragEntity) {
            let cartesian: Cartesian3 | undefined = viewer.scene.pickPosition(movement.endPosition)
